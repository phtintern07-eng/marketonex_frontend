document.addEventListener('DOMContentLoaded', () => {
    ...
    const savedTheme = localStorage.getItem('marketplace-theme') || 'dark';
    setTheme(savedTheme);

    function setTheme(theme) {
        // Apply to <html> (documentElement) so CSS [data-theme] selectors match
        document.documentElement.setAttribute('data-theme', theme);
        try { localStorage.setItem('marketplace-theme', theme); } catch (e) { }
        ...
        addProductBtn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                if (!authStatus.isAuthenticated) {
                    ...
                    sessionStorage.setItem('returnAfterLogin', 'add_product_marketplace.html');
                ...
                window.location.href = 'add_product_marketplace.html';
    ...
    window.editProduct = async function (productId, event) {
        ...
        window.location.href = 'add_product_marketplace.html?edit=' + productId;
    };
    ...
    window.deleteProduct = async function (productId, productName, event) {
        ...
        try {
            ...
            const baseUrl = window.API_BASE_URL || '';
            const response = await fetch(`${baseUrl}/api/marketplace/products/${productId}`, {
                method: 'DELETE',
            ...
            try {
                ...
                if (typeof ApiService !== 'undefined') {
                    try {
                        ...
                        await ApiService.request('/marketplace/products', 'POST', backendData);
                    } catch (apiError) {
                ...
                await MarketplaceStorage.saveProduct(productData);



                // Show success message
                alert('Product Added Successfully! Your product is now visible in the marketplace and saved to the database.');
                ...
                // Close modal (only if it exists - for marketplace.html)
                if (productFormModal) {
                    productFormModal.classList.add('hidden');
                    document.body.style.overflow = '';
                }
    ...
    function getVendors() {
        let savedVendors = localStorage.getItem('marketplaceVendors');
        if (!savedVendors) {
            // Seed localStorage if empty
            localStorage.setItem('marketplaceVendors', JSON.stringify(initialVendorsData));
        ...
        fetch(`${baseUrl}/api/feedback/status/${productData.id}`, {
            ...
            .then(statusData => {
                if (!statusData.is_logged_in) {
                    actionContainer.innerHTML = `<span style="font-size:0.85rem; color:#ef4444;"><i class="fas fa-info-circle"></i> Please <a href="../marketplace/login.html" style="color:var(--primary-color); text-decoration:underline;">login</a> to review</span>`;
                    ...
                    actionContainer.innerHTML = `<a href="../vendor/feedback.html?product_id=${productData.id}&product_type=${productData.product_type || 'marketplace'}" 
    ...
    window.addEventListener('feedbackSubmitted', async (event) => {
        ...
        try {
            ...
            console.error('Error updating marketplace after feedback:', error);
        }
    });
