        ...
        if (res.isAuthenticated) {
            ...
            if (currentUser.role === 'vendor') {
                ...
                if (!currentUser.verified || currentUser.verified === 'false' || currentUser.verified === 0) {
                    const verificationPages = ['signup.html', 'verification.html', 'vender_profile.html', 'loginvender.html'];
            ...
            if (path.includes('products.html') || path.includes('vender_profile.html')) {
            ...
            // Protected Routes
            const protectedPages = ['vender_profile.html', 'profile.html', 'products.html', 'orders.html', 'settings.html', 'insights.html', 'user-info.html', 'add-product.html', 'verification.html'];
...
function updateUIWithUser(user) {
    ...
    if (bizStatusBadge && user.biz_verification_status) {
        ...
        if (window.location.pathname.includes('vender_profile.html')) {
        ...
        container.innerHTML = `
            <div style="background: rgba(239, 68, 68, 0.05); border: 1px solid rgba(239, 68, 68, 0.1); border-radius: 12px; padding: 1.25rem;">
                ...
                <div style="margin-top: 1.25rem; border-top: 1px solid rgba(239, 68, 68, 0.1); padding-top: 1rem;">
                    <a href="biz_verification.html" class="update-btn" style="width: 100%; text-decoration: none; justify-content: center;">
        ...
        lines.forEach(line => {
            if (line.includes(']:') && !line.includes('REUPLOADED')) {
                ...
                item.innerHTML = `
                    ...
                    <a href="biz_verification.html" style="background: #ef4444; color: white; padding: 6px 12px; border-radius: 8px; font-size: 0.75rem; text-decoration: none; font-weight: 700; white-space: nowrap; transition: 0.2s; box-shadow: 0 4px 6px -1px rgba(239, 68, 68, 0.2);">
    ...
    function attachGlobalCardListener(card) {
        ...
        if (publishBtn) {
            publishBtn.addEventListener('click', async (e) => {
                ...
                if (confirm('Publish this product to the marketplace?')) {
                    try {
                        publishBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Publishing...';
                        const res = await fetch(`${window.API_BASE_URL || '/api'}/marketplace/products/${pid}/publish`, {
                            method: 'POST',
        ...
        if (unpublishBtn) {
            unpublishBtn.addEventListener('click', async (e) => {
                ...
                if (confirm('Unpublish this product from the marketplace?')) {
                    try {
                        unpublishBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Processing...';
                        const res = await fetch(`${window.API_BASE_URL || '/api'}/marketplace/products/${pid}/unpublish`, {
                            method: 'POST',
        ...
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (res.redirect) {
                    ...
                    window.location.href = 'marketplace.html?openAddProduct=true';
                } else {
