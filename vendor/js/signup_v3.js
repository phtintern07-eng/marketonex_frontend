...
async function safeFetch(url, options) {
    ...
    if (contentType.includes('application/json')) {
        ...
        // Response is HTML or empty - read text for debugging
        const text = await response.text().catch(() => '');
        console.warn("[signup_v3] Non-JSON response (" + response.status + "):", text.substring(0, 200));
        data = { error: 'Server error (' + response.status + '). Please try again or contact support.' };
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sendVerificationBtn) {
        sendVerificationBtn.addEventListener('click', async () => {
            ...
            try {
                const data = await safeFetch(BASE + '/auth/send-email-verification', {
                    method: 'POST',
    ...
    if (checkVerificationBtn) {
        checkVerificationBtn.addEventListener('click', async () => {
            ...
            try {
                const data = await safeFetch(BASE + '/auth/check-email-verification', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ email })
                });
